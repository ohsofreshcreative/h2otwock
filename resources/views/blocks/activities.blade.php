...
<section
	...
	@class([ 'b-activities relative -smt' ,
	...
	<div class="__wrapper c-main flex flex-col gap-8">
		@if (!empty($g_activities['header']))
		<h2 data-gsap-element="header" class="text-h2 text-primary">{{ $g_activities['header'] }}</h2>
		@endif

		@if (!empty($activities))
		<div class="swiper activities-swiper !overflow-visible relative">
			<div class="swiper-wrapper">
				@foreach ($activities as $item)
				<a
					href="{{ $item['link']['url'] ?? '#' }}"
					data-gsap-element="stagger"
					class="__card group swiper-slide !h-auto min-h-[464px] relative radius overflow-hidden flex flex-col justify-end px-10 pt-80 pb-14">
